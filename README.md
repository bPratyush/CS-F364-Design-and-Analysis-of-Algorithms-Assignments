...
```
For using ```g++``` compiler, replace ```clang++``` with ```g++``` and for changing C++ version, change the value in ```-std=c++17``` or remove for default version execution present in the system
